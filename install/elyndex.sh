...
# Download everything first so an existing pet is not touched after a network failure.
...
# Refuse to install if the downloaded manifest is not for this pet.
if ! grep -Eq '"id"[[:space:]]*:[[:space:]]*"elyndex"' "$new_pet_dir/pet.json"; then
...
if [[ -e "$install_path" ]]; then
	...
	# Keep backups as siblings and avoid nesting into an existing backup folder.
...
# Move the complete prepared folder into place in one step.
